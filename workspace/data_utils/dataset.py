...
class DataLoader:
    def __init__(self, data_name, args):
        ...
        if data_name == "chgh":
            from data_utils.data_loader_utils import get_chgh_data_dicts
            self.get_data_dicts = get_chgh_data_dicts
        else:
...
def get_infer_loader(keys, args):
    ...
    transform = importlib.import_module(f'augmentation.{args.data_name}_transform')
    get_inf_transform = getattr(transform, 'get_inf_transform', None)
    inf_transform = get_inf_transform(keys, args)
    inf_loader = get_dl(
        files=data_dicts,
