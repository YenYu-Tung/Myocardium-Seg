...
# chgh dataset helper 
def get_chgh_data_dicts(data_dir):
    """Build data_dicts for chgh dataset. Expects patient??? and patient???_gt files in subfolders."""
    patient_dirs = ['pid_02', 'pid_31', 'pid_1000']
    data_dicts = []
    for patient_dir in patient_dirs:
        data_dicts.append({
            "image": os.path.join(data_dir, patient_dir, f'{patient_dir}.nii.gz'),
            "label": os.path.join(data_dir, patient_dir, f'{patient_dir}_gt.nii.gz')
        })
    return data_dicts
