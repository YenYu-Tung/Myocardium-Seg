...
def check_channel(inp):
    # check shape is 5
    add_ch = EnsureChannelFirst(channel_dim="no_channel")
    len_inp_shape = len(inp.shape)
    if len_inp_shape in (3, 4):
        inp = add_ch(inp)
    return inp
...
def eval_label_pred(data, cls_num, device):
    ...
    val_labels_convert = []
    val_output_convert = []
    post_pred = AsDiscrete(to_onehot=cls_num)
    for val_label_tensor, val_pred_tensor in zip(val_label, val_pred):
        if val_label_tensor.shape[0] != 1:
            val_label_tensor = val_label_tensor.unsqueeze(0)
        if val_pred_tensor.shape[0] != 1:
            val_pred_tensor = val_pred_tensor.unsqueeze(0)
        val_labels_convert.append(post_label(val_label_tensor))
        val_output_convert.append(post_pred(val_pred_tensor))
...
def get_label_transform(data_name, keys=['label']):
    transform = importlib.import_module(f'augmentation.{data_name}_transform')
...
def run_infering(
    ...
    # ensure filename metadata exists for saving outputs
    if "image_meta_dict" not in data or "filename_or_obj" not in data.get("image_meta_dict", {}):
        fallback_name = getattr(args, "img_pth", None) or getattr(args, "pred_pth", None)
        if fallback_name:
            data["image_meta_dict"] = {"filename_or_obj": fallback_name}
        elif "pred_meta_dict" in data and "filename_or_obj" in data["pred_meta_dict"]:
            data["image_meta_dict"] = {"filename_or_obj": data["pred_meta_dict"]["filename_or_obj"]}
    if "label_meta_dict" not in data or "filename_or_obj" not in data.get("label_meta_dict", {}):
        fallback_label = getattr(args, "lbl_pth", None)
        if fallback_label:
            data["label_meta_dict"] = {"filename_or_obj": fallback_label}
